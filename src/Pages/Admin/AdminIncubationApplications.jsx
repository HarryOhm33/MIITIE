...
const AdminIncubationApplications = () => {
  ...
  const fetchApplications = async () => {
    try {
      setLoading(true);
      const querySnapshot = await getDocs(collection(db, "incubation_applications"));
      ...
      toast.error("Failed to load incubation applications");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleToggleStatus = async (app) => {
    ...
    try {
      await updateDoc(doc(db, "incubation_applications", app.id), {
        status: newStatus,
      });
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      await deleteDoc(doc(db, "incubation_applications", deletingApp.id));
  ...
  const filteredApps = applications.filter((app) => {
    const matchesSearch =
      ...
      app.startupIdea?.toLowerCase().includes(searchQuery.toLowerCase());
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-xl font-bold text-slate-800">Incubation Applications</h3>
          </div>
          <p className="text-xs text-slate-400 mt-1">Review startup proposals and incubation enrollment requests</p>
        </div>
        ...
        {deletingApp && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <h3 className="text-lg font-bold text-slate-800">Delete Incubation Proposal?</h3>
              <p className="text-slate-500 text-sm mt-2 leading-relaxed">
                Are you sure you want to delete proposal <span className="font-semibold text-slate-700">"{deletingApp.projectTitle || deletingApp.name}"</span>? This action cannot be undone.
              </p>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              <div className="flex justify-between items-start mb-4 border-b border-slate-100 pb-4">
                <div>
                  <h3 className="text-xl font-bold text-slate-800">{selectedApp.projectTitle || "Incubation Proposal"}</h3>
                  <p className="text-xs text-orange-500 font-medium">By {selectedApp.name}</p>
                </div>
                <button
                  onClick={() => setSelectedApp(null)}
              ...
              <div className="space-y-4 text-sm text-slate-600">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 bg-slate-50 p-4 rounded-xl">
                  ...
                  <div>
                    <span className="text-xs font-semibold text-slate-400 block">Mobile</span>
                    <span className="text-slate-800 font-medium">{selectedApp.mobile || "Not provided"}</span>
                  </div>
                  <div>
                    <span className="text-xs font-semibold text-slate-400 block">Stage</span>
                    <span className="text-slate-800 font-medium capitalize">{selectedApp.stage || "Idea"}</span>
                  </div>
                  <div>
                    <span className="text-xs font-semibold text-slate-400 block">Team Members</span>
                    <span className="text-slate-800 font-medium">{selectedApp.teamMembers || "1"}</span>
                  </div>
                  {selectedApp.fundingNeeded && (
                    <div className="sm:col-span-2">
                      <span className="text-xs font-semibold text-slate-400 block">Funding Requirement</span>
                      <span className="text-slate-800 font-medium">{selectedApp.fundingNeeded}</span>
                    </div>
                  )}
                  {selectedApp.address && (
                    <div className="sm:col-span-2">
                      <span className="text-xs font-semibold text-slate-400 block">Address / Location</span>
                      <span className="text-slate-800 font-medium">{selectedApp.address}</span>
                    </div>
                  )}
                </div>

                <div>
                  <span className="text-xs font-semibold text-slate-400 block mb-1">Startup Idea / Description</span>
                  <p className="p-4 bg-slate-50 rounded-xl leading-relaxed whitespace-pre-wrap text-slate-700">
                    {selectedApp.startupIdea || "No idea description provided."}
                  </p>
                </div>
              </div>
      ...
      {loading ? (
          ...
          {filteredApps.map((app) => (
            ...
            >
              <div>
                ...
                <h4 className="text-lg font-bold text-slate-800 mb-1">{app.projectTitle || "Incubation Proposal"}</h4>
                <p className="text-xs text-orange-500 font-medium mb-3">Applicant: {app.name}</p>
                ...
                <div className="space-y-2 text-xs text-slate-600 bg-slate-50 p-3 rounded-xl mb-4">
                  ...
                  {app.stage && (
                    <div className="flex items-center gap-2 capitalize">
                      <FaLightbulb className="text-slate-400 shrink-0" />
        ...
        <div className="bg-white border border-slate-100 rounded-2xl p-12 text-center text-slate-500 shadow-sm">
          No incubation applications found matching your search.
        </div>
      )}
    </div>
  );
...
export default AdminIncubationApplications;
